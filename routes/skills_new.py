...
def manage_variants(skill_id):
    """Manage variants for a specific skill"""
    try:
        ...
        flash(f'Error loading skill variants: {str(e)}', 'error')
        return redirect(url_for('skill.manage_skills'))
...
def create_skill():
    ...
    if request.method == 'GET':
        categories = SkillCategory.query.all()
        return render_template('admin/create_skill.html', categories=categories)
...
@skill_new_bp.route('/api/skills/extract', methods=['POST'])
...
@skill_new_bp.route('/api/skills/audit', methods=['POST'])
